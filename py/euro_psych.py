    ...
    for i, entry in enumerate(sorted(psych[eventId].items(), key=lambda e: e[1])):
      if not passed_qualification:
        ...
        if passed_qualification:
          ...
          print()
      if entry[1] != NA:
        print('%d. %s ([%s](https://worldcubeassociation.org/persons/%s)) (%s)' % (i+1, person_names[entry[0]], entry[0], entry[0], formatters.FormatTime(entry[1], eventId, 'bf' not in eventId and 'fm' not in eventId)))
      else:
